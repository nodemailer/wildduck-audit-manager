...
const db = require('./db');
const { addToStream } = require('./stream');
...
const authenticate = async (username, password, session, ip) => {
    ...
    if (username === config.root.username) {
        ...
        if (crypto.createHash('sha256').update(password).digest('hex').toLowerCase().trim() === userData.passwordHash.toLowerCase().trim()) {
            ...
            await addToStream(config.root.username, false, 'login', { ip });
        ...
        if (verified) {
            ...
            await addToStream(userData._id, false, 'login', { ip });
            return userData;
        }
    }
...
module.exports.logout = (req, res, next) => {
    ...
    req.logout(err => {
        if (err) {
            logger.error({ msg: 'Failed to log out', username, err });
            if (next) {
                return next(err);
            }
            return res.redirect('/');
...
module.exports.requireRoot = (req, res, next) => {
    if (!req.user) {
        return res.redirect(`/login`);
    }
