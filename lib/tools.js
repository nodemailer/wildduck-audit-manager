...
let signingKey;
openpgp.key
    .readArmored(signingKeyFile)
    .then(armored => {
        signingKey = armored.keys[0];
        return signingKey.decrypt(config.app.pgp.sign.password);
    })
    .then(() => {
        signFinger = formatFingerprint(signingKey.primaryKey.fingerprint).split(':').slice(-8).join('').toUpperCase();
    })
    .catch(err => {
        throw err;
    });
...
module.exports = { asyncifyRequest, asyncifyJson, validationErrors, getKeyInfo, checkPubKey, encrypt, signFinger: () => signFinger };
