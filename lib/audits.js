...
class Audits {
    ...
    async resolveDeletedUser(account) {
        return await this.userHandler.asyncGetDeleted(account, { username: true, address: true, name: true });
    }
    ...
    async createCredentials(values) {
        ...
        const groupData = await db.client.collection('auditgroups').findOne({ _id: values.audit, deleted: false, expires: { $gt: now } });
        if (!groupData) {
            return false;
        }
        ...
        const text = [
            ['Audit', 'Username', 'Password', 'Generated', 'Expires', 'Access URL'],
            [groupData.name, values.username, password, values.created.toISOString(), targetData.expires.toISOString(), config.app.clientUrl]
        ];

        let data = text.map(row => row.map(col => `"${(col || '').toString().replace(/\s+/g, ' ').trim().replace(/"/g, '""')}"`).join(',')).join('\n');
    ...
    async getCredentialDetails(id) {
        ...
        const r = await db.client.collection('auditusers').findOne({
            _id: id,
            deleted: false,
            level: { $in: ['audit', 'group'] }
        });
