...
class Audits {
    ...
    async getCredentials(id) {
        ...
        const credentials = await db.client.collection('auditusers').findOne({ _id: id, deleted: false });
        if (!credentials) {
            return false;
        }
    ...
    async deleteCredentials(id) {
        ...
        const r = await db.client.collection('auditusers').updateOne({ _id: id, deleted: false }, { $set: { deleted: true, deletedTime: new Date() } });
