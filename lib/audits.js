...
class Audits {
    ...
    getAuditDisplay(auditData) {
        ...
        return {
            ...
            start: auditData.start ? auditData.start.toISOString().substr(0, 10).replace(/-/g, '/') : '',
            end: auditData.end ? auditData.end.toISOString().substr(0, 10).replace(/-/g, '/') : '',
            expires: auditData.expires ? auditData.expires.toISOString().substr(0, 10).replace(/-/g, '/') : '',
    ...
    async get(id) {
        ...
        const auditData = await db.client.collection('audits').findOne({ _id: id });
        if (!auditData) {
            return false;
        }
    ...
    async createCredentials(values) {
        ...
        values.username = passwordGenerator.generate({
            length: 18,
            numbers: true,
            lowercase: true,
    ...
    async listCredentials(audit) {
        return await db.client.collection('auditusers').find({ audit, deleted: false }).sort({ _id: 1 }).toArray();
    }
    ...
    async getCredentials(id) {
        ...
        const credentials = await db.client.collection('auditusers').findOne({ _id: id });
        if (!credentials) {
            return false;
        }
