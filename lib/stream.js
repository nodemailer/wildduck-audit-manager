...
const formatStreamEntry = entryData => {
    let display = {
        ...
        auditType: /_group$/.test(entryData.action) ? 'group' : 'audit'
    };
    ...
    switch (entryData.action) {
        ...
        case 'create_audit_group':
            display.action = {
                name: 'Create audit',
                key: entryData.action
            };

            display.auditAccounts = [].concat(entryData.metadata.auditAccounts || []).filter(a => a && a.address);
            display.auditName = entryData.metadata.name;

            break;

        case 'create_audit_creds_group':
            display.action = {
                name: 'Create audit user',
                key: entryData.action
            };
            display.name = entryData.metadata.name;
            break;
